abstract type AbstractProjectedModel{L,P,S,N} <: AbstractModel{L,P,S,N} end

function (model::AbstractProjectedModel)(
    u0,
    g0,
    times,
    θ,
    ...
    (; lux_model, state, normalization, system, backend) = model
...
# FIRST ORDER AUTONOMOUS PROJECTED NEURAL ODE
struct FirstOrderAutonomousProjectedNeuralODE{L,P,S,N,Sys,B} <: AbstractProjectedModel{L,P,S,N}
    lux_model::L
    params::P
    state::S
    normalization::N
...
function FirstOrderAutonomousProjectedNeuralODE(
    dim,
    hidden_layers,
    hidden_width,
    activation,
...
# SECOND ORDER AUTONOMOUS PROJECTED NEURAL ODE
struct SecondOrderAutonomousProjectedNeuralODE{L,P,S,N,Sys,B} <:
       AbstractProjectedModel{L,P,S,N}
    lux_model::L
    params::P
    state::S
    normalization::N
...
function SecondOrderAutonomousProjectedNeuralODE(
    dim,
    hidden_layers,
    hidden_width,
    activation,
